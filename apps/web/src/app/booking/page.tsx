...
export default function BookingPage() {
  ...
  const [barbershopLoadingId, setBarbershopLoadingId] = useState<string | null>(null);
  ...
  const handleSelectBarbershop = async (shop: Barbershop) => {
    ...
    setBarbershopLoadingId(shop.id);
    setErrorMsg('');
  ...
  const handleConfirmBooking = async (e: React.FormEvent) => {
    ...
    if (!dateTime) {
      setErrorMsg('Por favor, selecione a data e o horário do agendamento.');
      return;
    }

    const token = localStorage.getItem('barber_token');
    if (!token) {
      setErrorMsg('Você precisa estar logado para finalizar o agendamento. Por favor, faça login.');
      return;
    }
  ...
  return (
    <div className="max-w-3xl mx-auto my-8">
      ...
      {errorMsg && (
        <div className="bg-red-500/10 border border-red-500 text-red-400 p-4 rounded-lg mb-6 text-sm font-medium flex justify-between items-center">
          <span>❌ {errorMsg}</span>
          <button onClick={() => setErrorMsg('')} className="font-bold px-2">✕</button>
        </div>
      )}
      ...
      {step === 1 && (
        <div className="space-y-4">
          <h2 className="text-xl font-semibold mb-4 text-white">Escolha a Barbearia:</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {barbershops.map((shop) => (
                <div
                  ...
                  className="p-5 bg-slate-900 border border-slate-800 hover:border-amber-500 rounded-xl cursor-pointer transition shadow-md flex justify-between items-center"
                >
                  <div>
                  ...
                  {barbershopLoadingId === shop.id && (
                    <span className="text-xs text-amber-400 font-bold animate-pulse">Carregando...</span>
                  )}
                </div>
              ))}
            </div>
      ...
      {step === 2 && (
        <div className="space-y-6">
          <div>
            <h2 className="text-xl font-semibold mb-3 text-white">Escolha o Serviço:</h2>
            {services.length === 0 ? (
              <p className="text-slate-400 text-xs">Nenhum serviço disponível nesta barbearia.</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {services.map((srv) => (
                  <div
          ...
          <div>
            <h2 className="text-xl font-semibold mb-3 text-white">Escolha o Barbeiro:</h2>
            {barbers.length === 0 ? (
              <p className="text-slate-400 text-xs">Nenhum barbeiro disponível no momento.</p>
            ) : (
          ...
          <div className="flex gap-4 pt-4">
            ...
            <button
              ...
              onClick={() => setStep(3)}
              className="px-6 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-500 disabled:opacity-50 font-medium text-sm"
            >
              Avançar
            </button>
          </div>
      ...
      {step === 3 && (
        <form onSubmit={handleConfirmBooking} className="bg-slate-900 border border-slate-800 p-6 rounded-xl space-y-4">
          <h2 className="text-xl font-semibold mb-4 text-white">Confirmar Agendamento</h2>
          ...
          <div className="flex gap-4 pt-4">
            ...
            <button
              ...
              className="px-6 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-500 disabled:opacity-50 font-medium text-sm"
            >
              {loading ? 'Confirmando Agendamento...' : 'Finalizar Agendamento'}
            </button>
          </div>
        </form>
      )}
      ...
      {step === 4 && (
        <div className="bg-slate-900 border border-slate-800 p-8 rounded-xl text-center space-y-4">
          ...
          <p className="text-slate-300">Seu horário foi reservado. Você pode acompanhar pelo seu painel.</p>
          <a
            href="/dashboard"
            className="inline-block mt-4 bg-amber-600 hover:bg-amber-500 text-white font-medium px-6 py-2.5 rounded-lg text-sm"
          >
